...
class Toprow:
    ...
    def create_tools_row(self):
        with gr.Row(elem_id=f"{self.id_part}_tools"):
            ...
            self.paste = ToolButton(value=paste_symbol, elem_id="paste", tooltip="Apply last generation parameters")
            self.clear_prompt_button = ToolButton(value=clear_prompt_symbol, elem_id=f"{self.id_part}_clear_prompt", tooltip="Clear prompt")
            self.apply_styles = ToolButton(value=ui_prompt_styles.styles_materialize_symbol, elem_id=f"{self.id_part}_style_apply", tooltip="Apply selected styles to prompt")

            if self.is_img2img:
                self.button_interrogate = ToolButton('📎', tooltip='Describe image with CLIP', elem_id="interrogate")
                self.button_deepbooru = ToolButton('📦', tooltip='Describe image with DeepBooru', elem_id="deepbooru")
